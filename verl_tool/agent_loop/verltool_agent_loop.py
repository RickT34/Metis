...
class VerlToolAgentLoop(AgentLoopBase):
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                async with session.post(
                    ...
                    if resp_data['response_size_mb'] > 2.0:
                ...
                traj_ids = payload.get('trajectory_ids', [])
                num_trajs = len(traj_ids) if isinstance(traj_ids, list) else 1
                
                logging.error(
                    f"[TIMEOUT] Attempt {attempt + 1}/{max_retries + 1} failed\n"
                    f"  Tool: {tool_name}\n"
                    f"  Timeout limit: {timeout_seconds}s\n"
                    f"  Elapsed time: {elapsed_time:.2f}s\n"
                    f"  Trajectories: {num_trajs} (ids: {traj_ids[:3]}{'...' if len(traj_ids) > 3 else ''})\n"
                    f"  Server URL: {self.agent_config.tool_server_url}\n"
                    f"  Error: {e}\n"
                    f"  {'Retrying...' if attempt < max_retries else 'Max retries reached, giving up.'}"
                )
                ...
                hostname = self.agent_config.tool_server_url.split('/')[2] if '//' in self.agent_config.tool_server_url else 'unknown'
                
                logging.error(
                    f"[CONNECTION ERROR] Attempt {attempt + 1}/{max_retries + 1} failed\n"
                    f"  Tool: {tool_name}\n"
                    f"  Elapsed time: {elapsed_time:.2f}s\n"
                    f"  Trajectories: {num_trajs} (ids: {traj_ids[:3]}{'...' if len(traj_ids) > 3 else ''})\n"
                    f"  Server URL: {self.agent_config.tool_server_url}\n"
                ...
                logging.error(
                    f"[GENERAL ERROR] Attempt {attempt + 1}/{max_retries + 1} failed\n"
                    f"  Tool: {tool_name}\n"
                    f"  Elapsed time: {elapsed_time:.2f}s\n"
                    f"  Trajectories: {num_trajs} (ids: {traj_ids[:3]}{'...' if len(traj_ids) > 3 else ''})\n"
                    f"  Server URL: {self.agent_config.tool_server_url}\n"
    ...
    async def send_batch_requests_async(self, batch_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Debug: Check payload size (only log if extremely large to reduce noise)
        try:
            json_str = json.dumps(safe_payload)
            payload_size_mb = len(json_str) / (1024 * 1024)
            if payload_size_mb > 10.0:  # Only warn if > 10MB (raised from 2MB)
                logger.warning(f"Large payload detected: {payload_size_mb:.2f} MB")
            elif payload_size_mb > 2.0:
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        for step in range(max_turns + 1):
            ...
            if isinstance(stop_reason, int):
                # FIX: [0] was taking the first CHARACTER of the decoded string, not the full token.
                # e.g. tokenizer.decode([151645]) = "<|im_end|>", [0] = "<" — WRONG!
                stop_reason = self.tokenizer.decode([stop_reason])
            ...
            if do_action and not is_last_step:
                ...
                if tool_results.get('image', None):
                    ...
                    # Decode images with error handling — skip corrupted/unidentifiable images
                    decoded_images = []
                    for img_url in images:
                        try:
                            decoded_images.append(decode_image_url(img_url))
                        except Exception as e:
                            logger.warning(f"Failed to decode image (skipping): {type(e).__name__}: {e}")
                    if not decoded_images:
                        # All images failed to decode — fall through to text-only branch
                        logger.warning(f"All {len(images)} images failed to decode. Treating observation as text-only.")
                        obs_text = obs_text.replace("<image>", "")  # remove any <image> tags
                        ...
                        stats_dict["valid_action"] += tool_results.get('valid_action', 0)
                    ...
                    elif num_image_tags > len(decoded_images):
                        # FIX: If obs_text has more <image> tags than actual images
                        # Remove ALL <image> tags first, then add the correct number
                        logger.warning(f"obs_text has {num_image_tags} <image> tags but only {len(decoded_images)} images provided. Normalizing to {len(decoded_images)} tags.")
                        obs_text = obs_text.replace("<image>", "")  # Remove all <image> tags
                        obs_text += "<image>" * len(decoded_images)  # Add correct number at the end
                        num_image_tags = len(decoded_images)
                    # now replace ALL <image> tags with image placeholder tokens
                    obs_text = obs_text.replace("<image>", self.qwen_image_placeholder)  # Replace ALL occurrences
                    obs_token_ids = self.processor(text=[obs_text], images=decoded_images, return_tensors="pt")["input_ids"].squeeze(0).tolist()
                    
                    # CRITICAL FIX: Truncate BEFORE adding to running_image_data
                    if max_obs_length < len(obs_token_ids):
                        if self.agent_config.truncate_obs_side == 'left':
                            ...
                            # Count how many vision placeholders were removed by truncation
                            truncated_part = obs_token_ids[truncation_index:]
                            vision_start_id = self.tokenizer.convert_tokens_to_ids("<|vision_start|>")
                            num_truncated_images = truncated_part.count(vision_start_id)
                            
                            # Remove truncated images from decoded_images list
                            if num_truncated_images > 0:
                                logger.warning(f"Observation truncation removed {num_truncated_images} images. Adjusting decoded_images list.")
                                decoded_images = decoded_images[:len(decoded_images) - num_truncated_images]
                    ...
                    # Validate: after truncation, the vision_start count in obs_token_ids should match decoded_images
                    vision_start_id_check = self.tokenizer.convert_tokens_to_ids("<|vision_start|>")
                    obs_vision_count = obs_token_ids.count(vision_start_id_check)
                    if obs_vision_count != len(decoded_images):
                        logger.error(f"Post-truncation validation failed: {obs_vision_count} vision blocks in obs_token_ids but {len(decoded_images)} decoded_images. "
                                     f"Adjusting decoded_images to match.")
                        decoded_images = decoded_images[:obs_vision_count]
                    
                    # Now add images to running_image_data (after potential truncation adjustment)
                    ...
                    elif num_audio_tags > len(decoded_audios):
                        # FIX: If obs_text has more <audio> tags than actual audios
                        # Remove ALL <audio> tags first, then add the correct number
                        logger.warning(f"obs_text has {num_audio_tags} <audio> tags but only {len(decoded_audios)} audios provided. Normalizing to {len(decoded_audios)} tags.")
                        obs_text = obs_text.replace("<audio>", "")  # Remove all <audio> tags
                        obs_text += "<audio>" * len(decoded_audios)  # Add correct number at the end
                        num_audio_tags = len(decoded_audios)
                    # now replace ALL <audio> tags with audio placeholder tokens
                    obs_text = obs_text.replace("<audio>", self.qwen_audio_placeholder)  # Replace ALL occurrences
                    # NOTE: Do NOT apply mtrl_sep here at text level; it's applied later at token level (line ~744).
                    # Applying it at both text and token level causes double wrapping.
                    obs_token_ids = self.processor(text=[obs_text], audio=decoded_audios, return_tensors="pt")["input_ids"].squeeze(0).tolist()
                    
                    # CRITICAL FIX: Truncate BEFORE adding to running_audio_data
                    if max_obs_length < len(obs_token_ids):
                        if self.agent_config.truncate_obs_side == 'left':
                            truncation_index = max_obs_length
                            if obs_token_ids[max_obs_length] in self.non_truncate_token_ids:
                            ...
                            # Count how many audio placeholders were removed by truncation
                            truncated_part = obs_token_ids[truncation_index:]
                            audio_start_id = self.tokenizer.convert_tokens_to_ids("<|audio_bos|>")
                            num_truncated_audios = truncated_part.count(audio_start_id)
                            
                            # Remove truncated audios from decoded_audios list
                            if num_truncated_audios > 0:
                                logger.warning(f"Observation truncation removed {num_truncated_audios} audios. Adjusting decoded_audios list.")
                                decoded_audios = decoded_audios[:len(decoded_audios) - num_truncated_audios]
                    ...
                    # Validate: after truncation, the audio_bos count in obs_token_ids should match decoded_audios
                    audio_start_id_check = self.tokenizer.convert_tokens_to_ids("<|audio_bos|>")
                    obs_audio_count = obs_token_ids.count(audio_start_id_check)
                    if obs_audio_count != len(decoded_audios):
                        logger.error(f"Post-truncation validation failed: {obs_audio_count} audio blocks in obs_token_ids but {len(decoded_audios)} decoded_audios. "
                                     f"Adjusting decoded_audios to match.")
                        decoded_audios = decoded_audios[:obs_audio_count]
                    
                    # Now add audios to running_audio_data (after potential truncation adjustment)
                ...
                if finish_reason == "stop":
                    if not stop_reason:
                        ...
                        # Model generated a tool call on the last step but we can't send it
                        stats_dict["is_traj_finished"] = False
                        traj_stop_reason = "max_turns_reached"
                    else:
                        # FIX: Model stopped due to EOS token (e.g. <|im_end|>), not an action stop token.
                        # This IS a natural finish — the model chose to end its response (e.g. after <answer>).
                        # Previously this was incorrectly set to False, which would cause mask_overlong_loss
                        # to mask these GOOD trajectories (the ones that actually produced an answer).
                        stats_dict["is_traj_finished"] = True
                        traj_stop_reason = "model_chose_to_finish_with_eos=" + str(stop_reason)
        ...
        # CRITICAL: Ensure image/audio data count matches the number of vision/audio placeholders in token IDs
        # This prevents vLLM from failing with "Failed to apply prompt replacement" errors
        if running_image_data is not None or running_audio_data is not None:
            ...
            if running_image_data is not None:
                ...
                if len(running_image_data) != num_visual_segments:
                    logger.warning(f"Image count mismatch: {len(running_image_data)} images vs {num_visual_segments} placeholders in tokens")
                    
                    if len(running_image_data) > num_visual_segments:
                        # Too many images: truncate to match placeholders
                        logger.warning(f"Truncating running_image_data from {len(running_image_data)} to {num_visual_segments}")
                        running_image_data = running_image_data[:num_visual_segments]
                    else:
                        # Too few images: remove extra vision blocks from response_ids
                        # instead of padding with wrong images (which causes image_grid_thw / image_pad mismatch)
                        num_extra = num_visual_segments - len(running_image_data)
                        logger.warning(f"Removing {num_extra} extra/phantom vision blocks from response to match {len(running_image_data)} images")
                        
                        vision_end_id = self.tokenizer.convert_tokens_to_ids("<|vision_end|>")
                        
                        # Find all vision block ranges in response_ids
                        vision_blocks_in_response = []
                        idx = 0
                        while idx < len(response_ids):
                            if response_ids[idx] == vision_start_id:
                                block_start = idx
                                j = idx + 1
                                while j < len(response_ids) and response_ids[j] != vision_end_id:
                                    j += 1
                                block_end = min(j + 1, len(response_ids))
                                vision_blocks_in_response.append((block_start, block_end))
                                idx = block_end
                            else:
                                idx += 1
                        
                        # Remove the last num_extra vision blocks from response_ids
                        blocks_to_remove = vision_blocks_in_response[-num_extra:] if len(vision_blocks_in_response) >= num_extra else vision_blocks_in_response
                        if blocks_to_remove:
                            ...
                            response_ids = [t for i, t in enumerate(response_ids) if i not in indices_to_remove]
                            response_mask = [t for i, t in enumerate(response_mask) if i not in indices_to_remove]
                            response_logprobs = [t for i, t in enumerate(response_logprobs) if i not in indices_to_remove]
                            logger.warning(f"Removed {len(indices_to_remove)} tokens from {len(blocks_to_remove)} vision blocks in response")
                        
                        # Verify the fix
                        new_full_ids = prompt_ids + response_ids
                        new_num_visual = new_full_ids.count(vision_start_id)
                        if new_num_visual != len(running_image_data):
                            logger.error(f"Failed to fix image count mismatch after vision block removal: {len(running_image_data)} images vs {new_num_visual} placeholders. Discarding multimodal data.")
                            running_image_data = None
            
        # Remove stray <|image_pad|> and <|video_pad|> tokens that are NOT inside vision blocks.
        # These can appear when the model generates them as hallucinated output, and they cause
        # "Image features and image tokens do not match" errors during training.
        vision_start_id = self.tokenizer.convert_tokens_to_ids("<|vision_start|>")
        vision_end_id = self.tokenizer.convert_tokens_to_ids("<|vision_end|>")
        ...
        for i, tok in enumerate(response_ids):
            if tok == vision_start_id:
                ...
                in_vision_block = False
            elif tok in (image_pad_id, video_pad_id) and not in_vision_block:
                stray_indices.add(i)

        if stray_indices:
            logger.warning(
                f"Found {len(stray_indices)} stray <|image_pad|>/<|video_pad|> tokens outside vision blocks in response_ids. "
                f"Removing them to prevent token/feature mismatch."
            )
            response_ids = [t for i, t in enumerate(response_ids) if i not in stray_indices]
            response_mask = [t for i, t in enumerate(response_mask) if i not in stray_indices]
            response_logprobs = [t for i, t in enumerate(response_logprobs) if i not in stray_indices]

        if running_image_data is not None or running_audio_data is not None:
            # Check audio placeholders
            if running_audio_data is not None:
                audio_start_id = self.tokenizer.convert_tokens_to_ids("<|audio_bos|>")
                full_ids = prompt_ids + response_ids  # recompute after potential image block removal
                num_audio_segments = full_ids.count(audio_start_id)
                
                if len(running_audio_data) != num_audio_segments:
                    logger.warning(f"Audio count mismatch: {len(running_audio_data)} audios vs {num_audio_segments} placeholders in tokens")
                    
                    if len(running_audio_data) > num_audio_segments:
                        logger.warning(f"Truncating running_audio_data from {len(running_audio_data)} to {num_audio_segments}")
                        running_audio_data = running_audio_data[:num_audio_segments]
                    else:
                        # Too few audios: remove extra audio blocks from response_ids
                        num_extra_audio = num_audio_segments - len(running_audio_data)
                        logger.warning(f"Removing {num_extra_audio} extra/phantom audio blocks from response")
                        
                        audio_end_id = self.tokenizer.convert_tokens_to_ids("<|audio_eos|>")
                        
                        # Find all audio block ranges in response_ids
                        audio_blocks_in_response = []
                        idx = 0
                        while idx < len(response_ids):
                            if response_ids[idx] == audio_start_id:
                                block_start = idx
                                j = idx + 1
                                while j < len(response_ids) and response_ids[j] != audio_end_id:
                                    j += 1
                                block_end = min(j + 1, len(response_ids))
                                audio_blocks_in_response.append((block_start, block_end))
                                idx = block_end
                            else:
                                idx += 1
                        
                        blocks_to_remove = audio_blocks_in_response[-num_extra_audio:] if len(audio_blocks_in_response) >= num_extra_audio else audio_blocks_in_response
                        if blocks_to_remove:
                            indices_to_remove = set()
                            for start, end in blocks_to_remove:
                                for i in range(start, end):
                            ...
                            response_mask = [t for i, t in enumerate(response_mask) if i not in indices_to_remove]
                            response_logprobs = [t for i, t in enumerate(response_logprobs) if i not in indices_to_remove]
                            logger.warning(f"Removed {len(indices_to_remove)} tokens from {len(blocks_to_remove)} audio blocks in response")
                        
                        # Verify the fix
                        new_full_ids = prompt_ids + response_ids
                        new_num_audio = new_full_ids.count(audio_start_id)
                        if new_num_audio != len(running_audio_data):
                            logger.error(f"Failed to fix audio count mismatch after audio block removal: {len(running_audio_data)} audios vs {new_num_audio} placeholders. Discarding multimodal data.")
                            running_audio_data = None
