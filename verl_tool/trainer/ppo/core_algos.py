...
def compute_grpo_conditional_advantage(
    ...
    """
    Computes GRPO-style advantage, but only considers samples where condition_mask is True
    for calculating the baseline (mean and std).
    
    This function's style is intentionally identical to `compute_grpo_outcome_advantage`.
    """
    scores = token_level_rewards.sum(dim=-1)
    ...
    with torch.no_grad():
        ...
        # Pass 1: group only samples that satisfy the condition
        for i in range(bsz):
            if scores[i] > 0:  # Only include scores that satisfy the condition in groups
                id2score[index[i]].append(scores[i])
...
def compute_hapo_advantage(
    ...
    # 1. For acc_reward, use the vanilla GRPO function.
    ...
    # 2. For tool_reward, use the conditional GRPO aligned with our style.
    ...
    # 3. Return two separate, correctly computed advantages
    return {
