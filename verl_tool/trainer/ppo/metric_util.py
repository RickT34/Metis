...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Log reward breakdown metrics (accuracy, answer_score, format_score, tool_score, num_turns, etc.)
    reward_metric_keys = ["accuracy", "answer_score", "format_score", "tool_score", "num_turns", "score"]
    for key in reward_metric_keys:
        if key in batch.non_tensor_batch:
            ...
            if len(values) > 0:
                result[f"reward/{key}/mean"] = values.mean()
                result[f"reward/{key}/max"] = values.max()
                result[f"reward/{key}/min"] = values.min()

    # Response length metrics — filter out 0s since 0 means "not applicable"
    # (e.g. correct_response_length=0 for wrong answers)
    for key in ["correct_response_length", "wrong_response_length"]:
        if key in batch.non_tensor_batch:
            values = batch.non_tensor_batch[key]
            values = np.array([float(v) for v in values if v is not None and float(v) > 0])
            if len(values) > 0:
                result[f"reward/{key}/mean"] = values.mean()
                result[f"reward/{key}/max"] = values.max()
                result[f"reward/{key}/min"] = values.min()
