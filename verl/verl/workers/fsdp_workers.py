...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def update_actor(self, data: DataProto):
        ...
        # Record when worker received the task
        t_receive = time.time()
        t_send = data.meta_info.get("__ray_send_time__", None)
        ...
        with self.ulysses_sharding_manager:
            ...
            t_out_transfer_start = time.time()
            output = output.to("cpu")
            t_out_transfer_end = time.time()
        ...
        # Get worker info
        import os
        worker_id = os.environ.get('RANK', 'unknown')
        
        # Add timing info to output (only from rank 0 to avoid conflicts when concatenating)
        if worker_id == '0':
            output.meta_info["__ray_receive_time__"] = t_receive
            output.meta_info["__ray_worker_done_time__"] = t_worker_done
        
        # Only print from rank 0 to reduce log noise
        if worker_id == '0':
            train_time = t_train_end - t_train_start
            total_time = t_end - t_start
            print(f"[Worker] update_actor: Train={train_time:.2f}s, Total={total_time:.2f}s", flush=True)
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        # Measure received data size (tensors)
        data_size_bytes = 0
        for key, value in data.batch.items():
            if hasattr(value, 'element_size') and hasattr(value, 'nelement'):  # torch.Tensor
                data_size_bytes += value.element_size() * value.nelement()
        ...
        for key, value in output.batch.items():
            if hasattr(value, 'element_size') and hasattr(value, 'nelement'):
                output_size_bytes += value.element_size() * value.nelement()
        ...
        if worker_id == '0':
            ...
            total_time = t_end - t_start
            print(f"[Worker] compute_log_prob: GPU={gpu_time:.2f}s, Total={total_time:.2f}s", flush=True)
